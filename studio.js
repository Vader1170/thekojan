...
function applyLayout() {
  ...
  gallery.className = `layout-${Math.min(n, 4)}`;
}
...
function addRemoteCell(peerId, label) {
  ...
  cell.innerHTML = `
    ...
    <div id="reconnect-overlay-${peerId}">
  ...
  // Insert before controls — gallery is the flex child, just append
...
function exitStudio() {
  ...
  document.body.innerHTML = '<div style="background:#080808;color:#888;font-family:\'DM Mono\',monospace;height:100vh;display:flex;align-items:center;justify-content:center;font-size:12px;letter-spacing:.1em;">SESSION ENDED</div>';
}
